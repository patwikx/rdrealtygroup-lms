...
// --- MODIFIED ---: Imported the new getLeaveTypes action
import { getLeaveBalances, getUserLeaveRequests, getLeaveTypes } from "@/lib/actions/leave-actions"
...
import { RequestsDataTable, RequestData } from "./components/request/components/request-table"
...
export default async function HomePage() {
  ...
  // --- MODIFIED ---: Added getLeaveTypes to the data fetching logic
  const [leaveRequestsResult, overtimeRequestsResult, leaveBalancesResult, leaveTypesResult] = await Promise.all([
    getUserLeaveRequests(userId),
    getUserOvertimeRequests(userId),
    getLeaveBalances(userId),
    getLeaveTypes() // Fetch the available leave types
  ]);
  
  const leaveRequests: RequestData[] = (leaveRequestsResult.success ? leaveRequestsResult.data : []).map(req => ({ ...req, type: 'leave' }));
  const overtimeRequests: RequestData[] = (overtimeRequestsResult.success ? overtimeRequestsResult.data : []).map(req => ({ ...req, type: 'overtime' }));
  
  const leaveBalances = leaveBalancesResult.success ? leaveBalancesResult.data : [];
  // --- MODIFIED ---: Get the leave types data from the result
  const leaveTypes = leaveTypesResult.success ? leaveTypesResult.data : [];
  
  const allRequests: RequestData[] = [...leaveRequests, ...overtimeRequests];

  const stats = {
    totalLeaveRequests: leaveRequests.length,
    totalOvertimeRequests: overtimeRequests.length,
    approvedRequests: allRequests.filter(r => r.status === RequestStatus.APPROVED).length,
    pendingRequests: allRequests.filter(r => 
      r.status === RequestStatus.PENDING_MANAGER || r.status === RequestStatus.PENDING_HR
    ).length,
    rejectedRequests: allRequests.filter(r => r.status === RequestStatus.REJECTED).length
  };
          ...
          <div className="@container/main flex flex-1 flex-col gap-2">
            <div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
                ...
                <Card className="w-full max-w-lg">
                  ...
                  <CardContent className="p-4">
                    <div className="space-y-3">
                      ...
                      {/* --- MODIFIED ---: Passed the `leaveTypes` prop to the dialog */}
                      <LeaveRequestDialog
                        userId={userId}
                        leaveTypes={leaveTypes} // Pass the fetched leave types
                        trigger={
              ...
              <div className="gap-4 space-y-6 px-4 lg:px-6">
                  {/* --- MODIFIED ---: Passed the `leaveTypes` prop to the data table */}
                  <RequestsDataTable
